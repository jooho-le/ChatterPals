...
(() => {
  ...
  // chrome.storage.local에 기반 URL이 저장되어 있으면 반영
  chrome.storage.local.get(['textApiBase', 'voiceApiBase'], (r) => {
    if (typeof r.textApiBase === 'string' && r.textApiBase.trim()) {
      ...
      console.log('[content] Using stored TEXT_API_BASE =', TEXT_API_BASE);
    }
    if (typeof r.voiceApiBase === 'string' && r.voiceApiBase.trim()) {
      ...
      console.log('[content] Using stored VOICE_API_BASE =', VOICE_API_BASE);
    }
  });
